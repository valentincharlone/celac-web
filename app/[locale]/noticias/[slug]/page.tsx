...
import { getNewsPost, getNewsPosts } from "@/lib/cms/news";
...
export default async function NoticiaPage({ params }: Props) {
  ...
  const related = (await getNewsPosts()).filter((p) => p.slug !== post.slug).slice(0, 3);
  ...
  const articleSchema = {
    ...
    /* Ahora la fecha es un dato real del CMS y no un texto localizado, así que el
       schema puede llevar `datePublished`. Sigue siendo opcional: las notas de
       archivo sin fecha en la fuente original no lo incluyen. */
    ...
    <>
      ...
      <article className="py-16 md:py-24 bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Los estilos del cuerpo viven en `globals.css` bajo `.payload-richtext`,
              que es el wrapper que escribe <RichText>. */}
          <RichText data={post.body[l]} />
